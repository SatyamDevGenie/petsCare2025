...
export default function Pets() {
  ...
  return (
    <div>
      <h1 className="text-3xl font-bold text-slate-900 mb-2">Pets</h1>
      <p className="text-slate-600 mb-8">Pets in our care.</p>
          ...
          <Link key={p._id} to={`/pets/${p._id}`}>
            <Card className="overflow-hidden h-full hover:shadow-md hover:border-primary-200 transition">
              ...
              <div className="p-4">
                ...
                <p className="text-sm text-slate-600">{p.type} • {p.breed}</p>
                <p className="text-sm text-slate-500">Age: {p.age} • {p.gender}</p>
              </div>
            </Card>
          </Link>
        ))}
      ...
      {(!list || list.length === 0) && (
        <p className="text-slate-500 text-center py-12">No pets listed.</p>
      )}
    </div>
  );
}

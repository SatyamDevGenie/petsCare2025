...
export default function ServicesManage() {
  ...
  const handleSubmit = (e) => {
    ...
    if (modal.edit) {
      dispatch(updateService({ id: modal.edit._id, ...payload })).then((r) => r.meta?.requestStatus === 'fulfilled' && closeModal());
    } else {
      dispatch(createService(payload)).then((r) => r.meta?.requestStatus === 'fulfilled' && closeModal());
    }
  };

  const handleDelete = (id) => {
    if (window.confirm('Delete this service?')) dispatch(deleteService(id));
  };
  ...
  return (
    <div>
      ...
      {(!list || list.length === 0) && !loading && (
        <Card><p className="text-gray-500 text-center py-8">No services. Add one above.</p></Card>
      )}

...
const updatePet = asyncHandler(async (req, res) => {
  try {
    ...
    // Allow admins to edit any pet, or allow users to edit their own pets
    if (!req.user.isAdmin && pet.user.toString() !== req.user._id.toString()) {
...
const deletePet = asyncHandler(async (req, res) => {
  try {
    ...
    // Allow admins to delete any pet, or allow users to delete their own pets
    if (!req.user.isAdmin && pet.user.toString() !== req.user._id.toString()) {
      return res.status(403).json({
        success: false,
...
const getVaccinationRecords = asyncHandler(async (req, res) => {
  try {
    ...
    // Allow admins to access any pet's vaccination records, or allow users to access their own pets' records
    if (!req.user.isAdmin && pet.user.toString() !== req.user._id.toString()) {
      return res.status(403).json({
        success: false,
